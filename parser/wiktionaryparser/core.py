...
from linguistics import PARTS_OF_SPEECH, RELATIONS
            ...
            while table and table.name not in ['h3', 'h4', 'h5']:
                ...
                if definition_tag.name in ['ol', 'ul']:
                    ...
                    for element in definition_tag.find_all('li', recursive=False):
                        if element.text:
                            sema_list[sema_counter] = element.text.strip()
                        sema_counter += 1
                    definition_text.append(sema_list)
    ...
    def parse_examples(self, word_contents):
        ...
        for def_index, def_id, def_type in definition_id_list:
            ...
            while table.name != 'ol':
                table = table.find_next_sibling()
            examples = {}
            while table and table.name in ['ol', 'ul']:
                sema_counter = 0
                for li in table.find_all('li', recursive=False):
                    sema_counter += 1
                    examples[sema_counter] = []
                    for dl in li.find_all('dl'):
                        example_by_sema = []
                        for element in dl.find_all('dd'):
                            example_text = re.sub(r'\([^)]*\)', '', element.text.strip())
                            if example_text:
                                example_by_sema.append(example_text)
                            element.clear()
                        examples[sema_counter] += example_by_sema
